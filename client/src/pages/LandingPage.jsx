...
import { Heart, Stethoscope, Calendar, Shield, Users, Award, Clock, Phone, Mail, MapPin, ChevronRight, Star, Activity } from 'lucide-react';
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-800">
          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-2 group">
              <div className="relative">
                <Heart className="h-8 w-8 text-blue-600 group-hover:scale-110 transition-transform duration-300" fill="currentColor" />
            ...
            <div className="relative group">
              ...
              <div className="relative bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-2xl">
                <div className="grid grid-cols-2 gap-4">
                  <StatCard icon={Users} value="50K+" label="Patients" delay="0s" />
                  <StatCard icon={Stethoscope} value="200+" label="Doctors" delay="0.1s" />
                  <StatCard icon={Award} value="25+" label="Awards" delay="0.2s" />
                  <StatCard icon={Clock} value="24/7" label="Support" delay="0.3s" />
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Services Section */}
      <section id="section-services" className="py-20 px-4 bg-white dark:bg-gray-800">
        <div className="max-w-7xl mx-auto">
          <div className={`text-center mb-16 transition-all duration-1000 ${isVisible['section-services'] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: Heart, title: 'Cardiology', desc: 'Expert heart care and cardiovascular treatments', color: 'red' },
              { icon: Activity, title: 'Neurology', desc: 'Advanced brain and nervous system care', color: 'purple' },
              { icon: Stethoscope, title: 'General Medicine', desc: 'Primary care for all ages', color: 'blue' },
              { icon: Shield, title: 'Pediatrics', desc: 'Specialized care for children', color: 'green' },
              { icon: Users, title: 'Family Medicine', desc: 'Holistic care for the whole family', color: 'orange' },
              { icon: Calendar, title: 'Preventive Care', desc: 'Regular check-ups and screenings', color: 'teal' },
            ].map((service, idx) => (
              <ServiceCard key={idx} {...service} delay={idx * 0.1} visible={isVisible['section-services']} />
            ))}
          </div>
        </div>
      </section>
      ...
      {/* Features Section */}
      <section id="section-features" className="py-20 px-4 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-900 dark:to-gray-800">
        <div className="max-w-7xl mx-auto">
          <div className={`grid lg:grid-cols-2 gap-12 items-center transition-all duration-1000 ${isVisible['section-features'] ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
            <div className="space-y-8">
              <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 dark:text-white">
                Why Choose Us?
              </h2>
              {[
                { icon: Award, title: 'Expert Doctors', desc: 'Board-certified physicians with decades of experience' },
                { icon: Shield, title: 'Advanced Technology', desc: 'State-of-the-art medical equipment and facilities' },
                { icon: Heart, title: 'Compassionate Care', desc: 'Patient-centered approach with personalized attention' },
                { icon: Clock, title: '24/7 Emergency', desc: 'Round-the-clock emergency services available' },
              ].map((feature, idx) => (
      ...
      {/* Testimonials */}
      <section id="section-testimonials" className="py-20 px-4 bg-white dark:bg-gray-800">
        <div className="max-w-7xl mx-auto">
          <div className={`text-center mb-16 transition-all duration-1000 ${isVisible['section-testimonials'] ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 dark:text-white mb-4">
              Patient Stories
            </h2>
            <p className="text-xl text-gray-600 dark:text-gray-400">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { name: 'Sarah Johnson', role: 'Patient', text: 'The care I received was exceptional. The staff was professional and caring.' },
              { name: 'Michael Chen', role: 'Patient', text: 'State-of-the-art facilities and amazing doctors. Highly recommend!' },
              { name: 'Emily Davis', role: 'Patient', text: 'They truly care about their patients. Best healthcare experience ever.' },
            ].map((testimonial, idx) => (
              <TestimonialCard key={idx} {...testimonial} delay={idx * 0.1} visible={isVisible['section-testimonials']} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <p className="text-xl text-blue-100 mb-8">
            Schedule your appointment today and experience healthcare like never before
          </p>
          <button 
            onClick={handleBookNow}
      ...
      <footer className="bg-gray-900 text-white py-16 px-4">
        <div className="max-w-7xl mx-auto grid md:grid-cols-4 gap-8">
          <div>
            ...
            <p className="text-gray-400">Providing exceptional healthcare services since 1995</p>
          </div>
          <div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-gray-400">
              ...
              <li className="hover:text-white transition-colors cursor-pointer">Diagnostics</li>
              <li className="hover:text-white transition-colors cursor-pointer">Pharmacy</li>
            </ul>
          </div>
          <div>
...
function StatCard({ icon: Icon, value, label, delay }) {
  return (
    <div className="bg-gradient-to-br from-blue-50 to-purple-50 dark:from-gray-700 dark:to-gray-600 p-6 rounded-2xl hover:scale-105 hover:shadow-xl transition-all duration-300 cursor-pointer" style={{ animationDelay: delay }}>
...
function ServiceCard({ icon: Icon, title, desc, color, delay, visible }) {
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 bg-gradient-to-br ${colorStyles[color]} opacity-0 group-hover:opacity-10 rounded-2xl transition-opacity duration-500`}></div>
      <div className={`inline-block p-4 rounded-xl bg-gradient-to-br ${colorStyles[color]} text-white mb-4 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
...
function FeatureItem({ icon: Icon, title, desc, delay }) {
  return (
    <div className="flex gap-4 items-start group cursor-pointer" style={{ animationDelay: `${delay}s` }}>
